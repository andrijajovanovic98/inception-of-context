...
def setup_bonus_dashboard(
    ...
    watcher: CodebaseWatcher = None,
    engine: PatchLoopEngine = None,
) -> None:
...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        .badge-amber {{ background: rgba(251, 191, 36, 0.15); color: var(--accent-amber); border-color: rgba(251, 191, 36, 0.3); }}
        .badge-purple {{ background: rgba(192, 132, 252, 0.15); color: var(--accent-purple); border-color: rgba(192, 132, 252, 0.3); }}
        ...
        .feed-header {{ display: flex; justify-content: space-between; font-size: 11px; color: var(--text-muted); margin-bottom: 4px; }}
        .feed-action {{ padding: 2px 6px; border-radius: 4px; font-size: 10px; font-weight: 700; }}
        .feed-action.CREATED, .feed-action.PATCH_SUCCESS, .feed-action.REINDEX, .feed-action.GIT_COMMIT {{ background: rgba(74, 222, 128, 0.2); color: var(--accent-green); }}
        .feed-action.MODIFIED, .feed-action.PATCH_START, .feed-action.DRY_RUN {{ background: rgba(251, 191, 36, 0.2); color: var(--accent-amber); }}
        ...
        .chunk-card {{ background: #090d16; border: 1px solid var(--border); border-radius: 6px; margin-bottom: 14px; overflow: hidden; }}
        .chunk-header {{ background: var(--surface); padding: 10px 14px; display: flex; justify-content: space-between; align-items: center; font-size: 13px; border-bottom: 1px solid var(--border); }}
        .code-pre {{ padding: 14px; font-family: monospace; font-size: 13px; overflow-x: auto; color: #f1f5f9; line-height: 1.4; max-height: 400px; }}
        .terminal-box {{ background: #000; color: #4ade80; border: 1px solid #1e293b; padding: 12px; font-family: monospace; font-size: 12px; border-radius: 6px; white-space: pre-wrap; max-height: 220px; overflow-y: auto; }}

        .attempt-card {{ background: var(--surface); border: 1px solid var(--border); border-radius: 8px; margin-bottom: 20px; overflow: hidden; }}
        .attempt-header {{ padding: 14px 18px; background: rgba(255,255,255,0.02); border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center; }}
        .attempt-body {{ padding: 18px; display: flex; flex-direction: column; gap: 16px; }}
        .sanity-grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 10px; margin-top: 6px; }}
        .sanity-pill {{ padding: 6px 10px; border-radius: 6px; font-size: 12px; display: flex; align-items: center; gap: 6px; border: 1px solid var(--border); background: #090d16; }}
        ...
        .error-feedback-box {{ background: rgba(248, 113, 113, 0.1); border: 1px solid rgba(248, 113, 113, 0.3); border-radius: 6px; padding: 12px; font-size: 13px; color: #fca5a5; white-space: pre-wrap; }}
        .feedback-note {{ font-size: 12px; color: var(--accent-amber); display: flex; align-items: center; gap: 6px; padding: 8px 12px; background: rgba(251, 191, 36, 0.1); border-radius: 6px; }}
...
<body>
    <div class="header">
        <div class="header-title">
            ...
            <span style="font-weight:600; font-size:14px; color:var(--accent-purple);">Bonus Part: Autonomous Engine</span>
        </div>
        <div style="display:flex; gap:10px; align-items:center;">
            <button class="btn btn-secondary" onclick="triggerOnDemandReindex()" style="padding:4px 12px; font-size:12px;" title="POST /reindex (Chapter VII)">
                ⟳ On-Demand Reindex
            </button>
    ...
    <div class="nav-tabs">
        ...
        <button class="tab-btn active" id="tab-btn-patch" onclick="showTab('patch')">Patch Loop &amp; Bonus</button>
    </div>
    ...
    <div id="tab-ask" class="tab-content">
        ...
        <div id="ask-loading" style="display:none; text-align:center; padding:30px; color:var(--accent);">Processing question via hybrid retrieval and Ollama...</div>
    ...
    <div id="tab-patch" class="tab-content active">
        <div class="form-panel">
            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:14px; flex-wrap:wrap; gap:10px;">
                <div>
                    <h2 style="font-size:16px; font-weight:700; color:#fff; margin-bottom:4px;">Autonomous Patch Loop &amp; Chapter VII Bonus Features</h2>
                    <p style="font-size:13px; color:var(--text-muted);">
                        Structured Patch &rarr; AST Sanity &rarr; Atomic Apply &rarr; Validation &rarr; Visual Diff &rarr; Auto-Commit / 100% Rollback
                    </p>
                </div>
                <div style="display:flex; gap:8px;">
                    <span class="badge" title="Active validation command in ioc.config.yml">Cmd: <code>{val_cmd}</code></span>
            ...
            <div class="form-group" style="margin-bottom:14px;">
                ...
                <textarea id="patch-intent" placeholder="e.g. Add a method multiply(a, b) to Calculator in calculator.py"></textarea>
            </div>
            ...
            <div style="display:flex; gap:20px; margin-bottom:14px; flex-wrap:wrap;">
                ...
                <label style="display:flex; align-items:center; gap:8px; cursor:pointer; color:var(--accent-green);">
    ...
    <script>
        ...
        async function triggerOnDemandReindex() {{
            try {{
                ...
                alert(`On-demand reindex complete! Total files: ${{data.total_files}}, total chunks: ${{data.total_chunks}}`);
        ...
        async function runBonusPatchLoop() {{
            ...
            statusTitle.innerText = dryRun ? 'Generating diff without modifying disk...' : 'Autonomous Patch Loop in progress...';
            statusMsg.innerText = 'Retrieving context &rarr; Generating JSON patch &rarr; Sanity check &rarr; Computing visual diff';
            container.innerHTML = '<div style="text-align:center; padding:40px; color:var(--accent);">Processing patch request...</div>';
        ...
        function renderBonusResult(result) {{
            ...
            if (result.dry_run) {{
                ...
                statusBadge.innerText = result.sanity_passed ? 'DRY-RUN: CLEAN DIFF' : 'DRY-RUN: SANITY REFUSAL';
                    ...
                    diffHtml += `
                        <div class="chunk-card">
                            <div class="chunk-header">
                                <span style="font-family:monospace; font-weight:700; color:var(--accent);">${{escapeHtml(d.path)}} (${{d.op}})</span>
                                <span style="font-size:12px; color:var(--text-muted);"><span style="color:var(--accent-green)">+${{d.additions}}</span> / <span style="color:var(--accent-red)">-${{d.deletions}}</span></span>
                            </div>
                            <div style="padding:10px;">${{d.html_diff}}</div>
                        </div>
                    `;
                }});
                container.innerHTML = diffHtml || '<div style="color:var(--text-muted);">No diff produced.</div>';
                return;
            }}
            ...
            if (result.status === 'success') {{
                ...
                statusMsg.innerText = result.error_message || 'Project cleanly restored to pre-patch snapshot.';
            }}
            ...
            (result.attempts || []).forEach(att => {{
                    ...
                    valHtml = `
                        <div>
                            <div style="display:flex; justify-content:space-between; margin-bottom:6px; font-size:12px;">
                                ...
                                <span class="badge ${{att.validation_exit_code === 0 ? '' : 'badge-red'}}">Exit Code: ${{att.validation_exit_code}}</span>
                            </div>
                            <pre class="terminal-box">${{escapeHtml(att.validation_output || '(no output)')}}</pre>
                        </div>
                    `;
                }}
                ...
                <div class="attempt-card">
                    <div class="attempt-header">
                        ...
                        <span style="font-size:12px; color:var(--text-muted);">${{escapeHtml((att.patch || {{}}).explanation || '')}}</span>
                    </div>
                    <div class="attempt-body">
                        <div>
                            <label>Visual Patch Diff (+ Additions / - Deletions):</label>
                            <div style="margin-top:8px;">${{diffsHtml || '<div style="color:var(--text-muted); font-size:13px;">No diff available.</div>'}}</div>
                        </div>
                        ${{valHtml}}
                    </div>
                </div>
        ...
        async function submitAsk(retrieveOnly) {{
            ...
            try {{
                ...
                document.getElementById('ground-truth-badge').style.display = data.pre_resolved ? 'inline-block' : 'none';
        ...
        function renderChunks(chunks) {{
            ...
            list.innerHTML = chunks.map((c, i) => `
                <div class="chunk-card">
                    <div class="chunk-header">
                        <span style="font-weight:700; color:var(--accent);">▶ #${{i+1}} ${{c.file_path}}</span>
                        <span class="badge">${{c.similarity_score ? (c.similarity_score*100).toFixed(1)+'%' : ''}}</span>
                    </div>
        ...
        async function loadFileChunks() {{
            ...
            document.getElementById('file-chunks-container').innerHTML = (data.chunks || []).map(c => `
                <div class="chunk-card">
                    <div class="chunk-header"><span style="font-weight:700; color:var(--accent);">▶ ${{c.symbol_name}} (${{c.symbol_type}})</span></div>
                    <pre class="code-pre"><code>${{escapeHtml(c.content)}}</code></pre>
                </div>
            `).join('');
        }}
        ...
        eventSource.onmessage = function(e) {{
            try {{
                ...
                if (feed && data.action !== 'CONNECTED') {{
                    ...
                    li.innerHTML = `<div class="feed-header"><span class="feed-action ${{data.action}}">${{data.action}}</span><span>${{data.timestamp || 'now'}}</span></div><div class="feed-path">${{data.path || ''}}</div><div>${{data.details || ''}}</div>`;
